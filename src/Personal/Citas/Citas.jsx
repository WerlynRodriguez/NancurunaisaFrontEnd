...
export default function Citas(props) {
  ...
  const getEvents = (date) => {
    ...
    if (Citas[`${date.date()}`])
      return (
        <Badge
          status={Estados[`${Citas[`${date.date()}`][0].idEstado}`].badge}
        />
      );
  };
    ...
    <div>
      ...
      <Layout className="ContentLayout">
        <Typography.Title level={4} style={{ marginLeft: "20px" }}>
          Citas del {selectedDate.date()} de {selectedDate.format("MMMM")} de{" "}
          {selectedDate.year()}
        </Typography.Title>
        ...
        <List
          ...
          renderItem={(cita, index) => (
            <ItemViewCita
              ...
              hora={cita.fechaHora}
              hab={
                cita.idHabitacionNavigation ? cita.idHabitacionNavigation : null
              }
              onClick={(id) => {
                onclickCita(id);
              }}
