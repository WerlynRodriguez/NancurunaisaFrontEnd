...
export const DropdownFilterMenu = (props) => (
  ...
  >
    <Button
      ...
      size="large"
    >
      <ControlOutlined style={{ fontSize: 20, color: "white" }} />
    </Button>
  </Dropdown>
);
    ...
    for (let i = 0; i < aux.length; i++) {
      if (aux[i].length < 3)
        return Promise.reject(
          new Error("¡El " + pos[i] + " " + text + " es muy corto!")
        );
      else if (aux[i].length > 15)
        return Promise.reject(
          new Error("¡El " + pos[i] + " " + text + " es muy largo!")
        );
    }
  ...
  return (
    ...
    >
      ...
      <div style={{ marginTop: "70px", textAlign: "center" }}>
        ...
        <Title level={2} style={{ display: Loading ? "None" : "" }}>
          {Text}
        </Title>

        <Title level={5} style={{ display: Loading ? "None" : "" }}>
          {SubTitle}
        </Title>
      </div>
    </Layout>
